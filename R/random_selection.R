...
#' @param vec_prob vector, the probility vector of vec_rate and 1-vec_rate
#'
#' @return results, int
#' @export

#' @examples random_selection(4, c(0.12, 0.88))
random_selection = function(freq, vec_prob) {
  ...
  for (i in 1:freq)
    ...
    every_selection[i] = sample(c(1, 0), 1, prob = vec_prob)
  # 加总所有链接的返回值，并返回
